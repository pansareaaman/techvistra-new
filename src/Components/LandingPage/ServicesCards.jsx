...
const ServicesCards = () => {
  return (
    ...
    <div className="bg-[#0d0c1d] z-10 relative -mt-20 mb-11 text-white pb-10  flex gap-6 lg-mx:px-5  md-mx:flex-col md-mx:w-full mx-auto ">

      <img className="w-52 h-5/6 xl-mx:w-40 xl-mx:h-4/6 lg-mx:hidden" src="/assets/LeftBox.png" alt="" />

      {services.map((service, index) => (
        <div key={index} className="bg-[#120819] p-2 rounded-lg shadow-lg hover:shadow-[#8c8ccd] text-center border w-2/6  md-mx:w-[70%] md-mx:mx-auto"> 
          <h3 className="text-lg font-bold underline ">{service.title}</h3>
          <p className="text-gray-400 py-4 px-5">{service.description}</p>
          {/* <div className="text-3xl mb-4">{service.icon}</div> */}
          <Avatar className="mx-auto" size='lg' src={`/assets/${service.icon}`}  / >        
          </div>
      ))}

      <img className="w-52 h-5/6 xl-mx:w-40 xl-mx:h-4/6 lg-mx:hidden" src="/assets/RightBox.png" alt="" />
    </div>
  );
};
